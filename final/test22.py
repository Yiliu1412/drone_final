...
def calibrate(flight: Flight, aruco_id, threshold, down=False):
    ...
    while True:
        ...
        distance_square = (cx - IR_CAMERA_CODE_CENTER[0]) ** 2 + (cy - IR_CAMERA_CODE_CENTER[1]) ** 2
        ...
        dx = x_pid(cx - IR_CAMERA_CODE_CENTER[0])
        dy = y_pid(cy - IR_CAMERA_CODE_CENTER[1])
        dx = outer_bound(dx, 15, 9)
        dy = outer_bound(dy, 15, 9)

        cv2.circle(image, (int(cx), int(cy)), 3, (255, 0, 0), -1)
        cv2.circle(image, (int(IR_CAMERA_CODE_CENTER[0]), int(IR_CAMERA_CODE_CENTER[1])), 3, (0, 255, 0), -1)
        cv2.circle(image, (int(cx + dx), int(cy + dy)), 3, (0, 0, 255), -1)
        cv2.imshow('video', image)
        cv2.waitKey(1)
        print('dcx={} dcy={} dx={} dy={}'.format(cx - IR_CAMERA_CODE_CENTER[0], cy - IR_CAMERA_CODE_CENTER[1], dx, dy))
...
print('[info] step 3')
flight.left(190).wait_for_completed(timeout=5)
...
while True:
    ...
    if aruco_results is None:
        flight.stop().wait_for_completed(timeout=1)
        time.sleep(1)
        flight.right(30).wait_for_completed(timeout=3)
        continue
    break
